...
func TestNewTimeAccumulator_InvalidMaxWaitTimeShouldErr(t *testing.T) {
	...
	ta, err := accumulator.NewTimeAccumulator(accumulator.MinimumAllowedTime-1, 0, &mock.LoggerFake{})
...
func TestNewTimeAccumulator_InvalidMaxOffsetShouldErr(t *testing.T) {
	...
	ta, err := accumulator.NewTimeAccumulator(accumulator.MinimumAllowedTime, -1, &mock.LoggerFake{})
...
func TestNewTimeAccumulator_NilLoggerShouldErr(t *testing.T) {
	...
	assert.True(t, check.IfNil(ta))
	assert.True(t, errors.Is(err, core2.ErrNilLogger))
}

func TestNewTimeAccumulator_ShouldWork(t *testing.T) {
	t.Parallel()

	ta, err := accumulator.NewTimeAccumulator(accumulator.MinimumAllowedTime, 0, &mock.LoggerFake{})
...
func TestTimeAccumulator_AddDataShouldWorkEvenIfTheChanIsBlocked(t *testing.T) {
	...
	ta, _ := accumulator.NewTimeAccumulator(allowedTime, 0, &mock.LoggerFake{})
	go func() {
		ta.AddData(struct{}{})
...
func TestTimeAccumulator_EvictionWithOffsetShouldPreserveTheOrder(t *testing.T) {
	...
	allowedTime := time.Millisecond * 100
	ta, _ := accumulator.NewTimeAccumulator(allowedTime, time.Millisecond, &mock.LoggerFake{})
...
func TestTimeAccumulator_ComputeWaitTimeShouldWork(t *testing.T) {
	...
	ta, _ := accumulator.NewTimeAccumulator(allowedTime, maxOffset, &mock.LoggerFake{})
