...
func TestNewBech32PubkeyConverter_InvalidSizeShouldErr(t *testing.T) {
	...
	bpc, err := pubkeyConverter.NewBech32PubkeyConverter(-1, &mock.LoggerFake{})
	assert.True(t, errors.Is(err, pubkeyConverter.ErrInvalidAddressLength))
	assert.True(t, check.IfNil(bpc))

	bpc, err = pubkeyConverter.NewBech32PubkeyConverter(0, &mock.LoggerFake{})
	assert.True(t, errors.Is(err, pubkeyConverter.ErrInvalidAddressLength))
	assert.True(t, check.IfNil(bpc))

	bpc, err = pubkeyConverter.NewBech32PubkeyConverter(3, &mock.LoggerFake{})
	assert.True(t, errors.Is(err, pubkeyConverter.ErrInvalidAddressLength))
	assert.True(t, check.IfNil(bpc))
}
...
func TestNewBech32PubkeyConverter_ShouldWork(t *testing.T) {
	...
	addressLen := 28
	bpc, err := pubkeyConverter.NewBech32PubkeyConverter(addressLen, &mock.LoggerFake{})
...
func TestBech32PubkeyConverter_DecodeInvalidStringShouldErr(t *testing.T) {
	...
	addressLen := 32
	bpc, _ := pubkeyConverter.NewBech32PubkeyConverter(addressLen, &mock.LoggerFake{})
