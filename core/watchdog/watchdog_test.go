...
func TestNewWatchdog_NilAlarmSchedulerShouldErr(t *testing.T) {
	...
	w, err := watchdog.NewWatchdog(nil, make(chan endProcess.ArgEndProcess, 1), &mock.LoggerFake{})
...
func TestNewWatchdog_NilChanShouldErr(t *testing.T) {
	...
	w, err := watchdog.NewWatchdog(&mock.AlarmSchedulerStub{}, nil, &mock.LoggerFake{})
	assert.True(t, check.IfNil(w))
...
func TestWatchdog_Set(t *testing.T) {
	...
	w, _ := watchdog.NewWatchdog(alarmScheduler, make(chan endProcess.ArgEndProcess, 1), &mock.LoggerFake{})
...
func TestWatchdog_SetDefault(t *testing.T) {
	...
	w, _ := watchdog.NewWatchdog(alarmScheduler, channel, &mock.LoggerFake{})
